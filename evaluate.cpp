...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strstr(argv[i], ".fits") != NULL) {
            std::auto_ptr<FITS> pInfile(new FITS(argv[i], Read, true));
            ExtHDU& image = pInfile->extension("Raw Frame");

            valarray<uint8_t> contents;
            image.read(contents);
            uint16_t width = image.axis(0);
            uint16_t height = image.axis(1);

            cv::Mat frame(height, width, CV_8UC1, &contents[0]);
